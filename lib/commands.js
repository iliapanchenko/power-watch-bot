...
function listPresets(table, current, describe) {
  return Object.entries(table)
    .map(([name, preset], index) => {
      const mark = name === current ? '▸' : ' ';
      return `${mark} <b>${index + 1}. ${name}</b> — ${describe(preset)}\n     <i>${preset.about}</i>`;
    })
    .join('\n');
}
...
async function setMode(chat, rest) {
  ...
  if (!rest) {
    return (
      ...
      `${shortSpan(reactionSeconds(current, tempo))}.\n\n` +
      listPresets(MODES, current, (preset) =>
        shortSpan(tempoSeconds(tempo) * preset.failThreshold)
      ) +
      '\n\nЗмінити: <code>/mode спокійний</code> або <code>/mode 3</code>'
    );
  }

  const chosen = resolveMode(rest);
  if (!chosen) {
    return `Такого режиму немає. Доступні: ${Object.keys(MODES).join(', ')}.\nАбо просто /mode, щоб побачити список.`;
  }
  ...
  return (
    `Режим: <b>${chosen}</b>\n\n` +
    `Тепер відключенням вважається тиша довша за ${shortSpan(reactionSeconds(chosen, tempo))}. ` +
    'Повернення світла фіксується так само швидко, як і раніше.'
  );
}

async function setTempo(rest) {
  const settings = await loadSettings();
  const current = resolveTempo(settings.tempo);

  if (!rest) {
    return (
      '<b>Темп опитування</b>\n\n' +
      `Зараз: <b>${current}</b> — раз на ${tempoSeconds(current)} секунд.\n\n` +
      listPresets(TEMPOS, current, (preset) => `раз на ${preset.seconds} с`) +
      '\n\nЗмінити: <code>/tempo частий</code> або <code>/tempo 1</code>\n' +
      '<i>Спільне налаштування: діє на всі чати одразу.</i>'
    );
  }
  ...
  return (
    `Темп: <b>${chosen}</b> — раз на ${tempoSeconds(chosen)} секунд.\n\n` +
    'Діє на всі чати. Пороги в кожного свої, тому реакція в них зміниться пропорційно.'
  );
}
